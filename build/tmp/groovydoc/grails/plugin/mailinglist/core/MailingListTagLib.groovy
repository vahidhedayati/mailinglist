...
class MailingListTagLib implements GrailsApplicationAware {
	...
	def translateCronExpress={attrs ->
		...
		if (ce && ce.contains(' ') && ce.split(' ',-1).length>5 && ce.contains(' ') && ce.split(' ',-1).length<7) {
			def c=ce.split(' ',-1)
			...
			// in standard cases mailinglist.cron.month.1=Jan etc 
			// exists but for complex binding you need to add yourself 
			// to your messages.properties
			if (month!=startTime&&month!=always) {
				sb.append(g.message(code:'mailinglist.cron.month',args:[months."${month}"?:month])+", ")
			}
			// in standard cases mailinglist.cron.dayOfWeek.1=Mon etc exists for complex 1-5 Mon-Fri add that yourself
			if (dayOfWeek!=startTime&&dayOfWeek!=always) {
				sb.append(g.message(code:'mailinglist.cron.dayOfWeek',args:[weekDays."${dayOfWeek}"?:month]))
			}	
			if (year) {
