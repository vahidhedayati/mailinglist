...
class TinyMceTagLib {
   ...
   /**
    * Just add TinyMce JS to your page
    */
   def resources = { attrs ->
      String tinyMce = g.resource(plugin: "tinyMce", dir: 'js/tinymce/jscripts/tiny_mce', file: "tiny_mce.js").encodeAsHTML();
      out << """<script type="text/javascript" src="${tinyMce}"></script>"""

      if (attrs.widget) {
         tinyMce = g.resource(plugin: "tinyMce", dir: 'js/tinymce/jscripts/tiny_mce', file: "jquery.tinymce.js").encodeAsHTML();
         out << """<script type="text/javascript" src="${tinyMce}"></script>"""
      }
   }

   /**
    * Adds an variable with the final location of the tinyMce JS file, to be loaded inside jquery implementation
    */
   def sourceJs = { attrs, body ->
      String tinyMce = g.resource(plugin: "tinyMce", dir: 'js/tinymce/jscripts/tiny_mce', file: "tiny_mce.js").encodeAsHTML();
      out << """<script type="text/javascript">var tinyMceSource = "${tinyMce}";</script>"""
   }
