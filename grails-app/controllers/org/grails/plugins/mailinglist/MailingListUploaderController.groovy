...
class MailingListUploaderController  {
    ...
    def upload() {
		...
		output +="${session.usersname} is upload a CSV - : Category name: "+params.catname+"<br/>"
		int i=0;
		
		def cid=MailingListCat.findOrSaveWhere(name: params.catname)
			...
			if (i>1) {
				...
				try {
					...
					if (rtype==1) {
						...
						if (!title) { title=''}
						if (!fname) { fname=''}
						def findlrh=MailingList.findByEmailAddressAndMlcategories(email,cid)
						if (!findlrh) {
					...
					def findlrh=MailingList.where {
						title==title && firstName==fname && middleName==mname && lastName==sname && suffix==suffix  && emailAddress==email && emailDisplayName==display				
					}
					if (!findlrh) {
	...
	def export() { 
		def mcat=MailingListCat.findById(params.id)
