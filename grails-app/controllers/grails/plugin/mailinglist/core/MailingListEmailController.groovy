...
@Transactional
class MailingListEmailController implements GrailsApplicationAware {
	...
	def scheduleEmail(ScheduleBaseBean bean) { 
		...
		flash.message = message(code: 'mailingList.could.not.queue', default: 'Could not queue job please check quartz queue to ensure schedule slots are free')
		render(view: view, model: [mailingListScheduleInstance: bean])
	}
